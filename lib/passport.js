...
passport.connect = function (req, query, profile, next) {
  sails.log.debug("Passport connect");
  var newUser = {}
    , provider;
  ...
  , identifier : query.identifier.toString()
    ...
    if (!req.user) {
      ...
      if (!passport) {
        var createUserRetries = 5;  // Allow creation retry x times if UUID doesn't match
        var createUserCb = function (err, user) {
          if (err) {
            if (err.code === 'E_VALIDATION') {
              ...
              else if (err.invalidAttributes.uuid && 
                  err.invalidAttributes.uuid.some(function(element, index, array) {
                    return element.rule === 'unique';
                  })) {
                // Catch error for when the uuid somehow doesn't match
                if (createUserRetries == 0) {
                  req.flash('error', req.__('Error.Passport.Unknown', err.code));
                  sails.log.error(err);
                } else {
                  sails.log.warn("UUID generated for user is not unique. Retries left: " + createUserRetries);
                  createUserRetries--;
                  User.create(newUser, createUserCb);
                  return;
                }
              } 
              else {
...
passport.endpoint = function (req, res) {
  var strategies = sails.config.userauth.providers
...
passport.callback = function (req, res, next) {
  ...
  if (provider === 'local' && action !== undefined) {
    ...
    if (action === 'disconnect' && req.user) {
      ...
      // has failed.
      this.authenticate(provider, function(err, user, info) {
        if (err) {
          next(err, user, info);
        }
        else {
...
passport.loadStrategies = function () {
  var self       = this
    , strategies = sails.config.userauth.providers;

  Object.keys(strategies).forEach(function (provider) {
    ...
    if (provider === 'local') {
      ...
      var protocol = strategies[provider].protocol
        , callback = strategies[provider].callback;

      if (!callback) {
        callback = path.join(sails.config.userauth.basepath, provider, 'callback');
      }
      ...
      // do that.
      _.extend(options, strategies[provider].options);
